...
**Fallback gate:** Script fallback (`scripts/smart_search.sh`, `scripts/search_mathlib.sh`) and repair agents are permitted when:
...
`${LEAN4_PYTHON_BIN:-python3} "scripts/sorry_analyzer.py" <target> --format=json --report-only`.
...
Autonomous commands (`autoprove`, `autoformalize`) use `scripts/cycle_tracker.sh` for deterministic session counter tracking. Guided commands (`prove`, `formalize`) do not — user presence provides the control loop.
...
```bash
bash "scripts/cycle_tracker.sh" init \
  --max-cycles=<resolved> \
  --max-stuck=<resolved> \
...
```bash
bash "scripts/cycle_tracker.sh" tick --stuck=yes|no
```
...
1. Call `bash "scripts/cycle_tracker.sh" can-deep`
...
3. If exit 0: call `bash "scripts/cycle_tracker.sh" deep` to record the invocation, then dispatch
...
- Call `bash "scripts/cycle_tracker.sh" start-claim` when dequeuing each claim
- Call `bash "scripts/cycle_tracker.sh" reset-claim` when a claim completes or stops (before the next `start-claim`)
...
Call `bash "scripts/cycle_tracker.sh" status` for the structured summary counters, then `bash "scripts/cycle_tracker.sh" stop` for cleanup.
