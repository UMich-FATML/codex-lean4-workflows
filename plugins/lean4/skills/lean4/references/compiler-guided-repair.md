...
python3 scripts/parse_lean_errors.py errors.txt > context.json
```
...
```bash
python3 scripts/solver_cascade.py context.json FILE.lean
```
...
**Error parsing:**
```bash
python3 scripts/parse_lean_errors.py errors.txt
```
...
**Fallback scripts:**
```bash
bash scripts/smart_search.sh "query" --source=all
```
